...
global labeling_percentage_entry, cell_compartment_label, cell_compartment_combobox
...
def id_deletion_of_root_outputs_with_errors():
    ...
    while indexe_of_root_output < (nb_complete_simulations * nb_files_for_one_complete_simulation):
        try:
            if (((indexe_of_root_output - 1) % nb_files_for_one_complete_simulation) == 0) & \
...
def main():
    ...
    nb_nucl_traversees_par_la_particule_tab_sur_toutes_simus, surviel_append_sur_une_simu,\
        ...
        dosec_append_sur_toutes_simus, dosem_append_sur_une_simu, dosem_append_sur_toutes_simus,\
    ...
    for i in range(index_of_first_root_output, len(indexes_root_files_without_errors)):
        ...
        simulation_id.append(i) #working only if no multithreading
        ...
        for h in range(hdepart, (hfin+1), nb_group_of_cells_considered):
            ...
            data, ind_alphaplusplus, ind_alphaplus, ind_helium, data_alpha, ind_end_of_run, data_edep_cell = \
            ...
            for ind_division_simus in range(0, nb_group_of_cells_considered):
                ...
                if indice_available_diffusion_info == 0:
                    data.append(np.core.records.fromarrays([d1, d2, d3, d4, d5, d6, d7, d8],
                    ...
                    data.append(np.core.records.fromarrays([d1, d2, d3, d4, d5, d6, d7, d8, d9],
                                names='nameParticle, Ei, Ef, ID_Cell, Cellule_D_Emission, eventID, fEdepn, fEdepc,'
                                      ' indice_if_diffusion'))
                elif indice_available_diffusion_info == 1 and indice_available_edep_sph_info == 1:
                    data.append(np.core.records.fromarrays([d1, d2, d3, d4, d5, d6, d7, d8, d9, d10],
                                names='nameParticle, Ei, Ef, ID_Cell, Cellule_D_Emission, eventID, fEdepn, fEdepc,'
                ...
                data_alpha.append((np.concatenate(((data[ind_division_simus])[ind_alphaplusplus[ind_division_simus]],
                                (data[ind_division_simus])[ind_alphaplus[ind_division_simus]],
                                (data[ind_division_simus])[ind_helium[ind_division_simus]]))))

                (data_alpha[ind_division_simus])["eventID"] +=\
                    (nb_cellules_reel / nb_group_of_cells_considered)*ind_division_simus

                ind_end_of_run.append((data[ind_division_simus])["nameParticle"] == 'EndOfRun')

                data_edep_cell.append((data[ind_division_simus])[ind_end_of_run[ind_division_simus]])
            ...
            energy_he=np.zeros(len(he[0]))
            for k in range(0,len(he[0])):
                energy_he[k] = (he[1][k] + he[1][k + 1]) / 2.
            ...
            progress_bar['value'] += 100 // math.floor(nb_complete_simulations)
    ...
    print(f"TCP = {tcp} +- {2*np.std(tcp_append_sur_toutes_simus_np)/np.sqrt(nb_complete_simulations)}", '\n'
          ...
          f"Max dose totale absorbée = {np.mean(np.max(dosen_c_append_sur_toutes_simus_np,axis=1))} +- "
                      f"{2 * np.std(np.max(dosen_c_append_sur_toutes_simus_np, axis=1))}", '\n'
          ...
          f"Sum dose cellules = {np.sum(dosen_c_tot)}", '\n'
...
def add_new_buttons_to_graphic_window():
    ...
    nb_complete_simulations, nb_files_for_one_complete_simulation, nb_group_of_cells_considered, simulation_name,\
